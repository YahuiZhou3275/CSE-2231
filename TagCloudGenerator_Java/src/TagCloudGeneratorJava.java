...
public final class TagCloudGeneratorJava {
    ...
    public static int sortWords(Map<String, Integer> storeWords, int num,
        ...
        Map.Entry<String, Integer> removed = countSort.remove(0);
        int min = removed.getValue();
        //TODO: would the order be changed?
        alphabetSort.add(removed);

        for (int i = 0; i < numToShow - 1; i++) {
            Map.Entry<String, Integer> remove = countSort.remove(0);

            int x = remove.getValue();
            if (x < min) {
                min = x;
            }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.err.println("Error receiving the input from keyboard.");
        }
        ...
        try {
            ...
            //TODO: check whether the output should be opening the output file or create an output file
        ...
        int numWord = 0;
        try {
            numWord = Integer.parseInt(in.readLine());
            while (numWord < 0) {
                System.out.println("The number should be non-negative."
                        + " Please reenter a new number:  ");
                //TODO: check whether it is valid (should we add try-catch for below?)
                numWord = Integer.parseInt(in.readLine());
            }
        } catch (IOException e) {
            System.err.println("Error receiving the input from keyboard.");
        ...
        // TODO: NEED TO FIX!!!!IOException is never reachable???
        try {
            outFile.close();
        } catch (IOException e) {
            System.err.println("Error closing the intput File");
        }
