...
public final class Queue1LSort3<T> extends Queue1L<T> {
    ...
    private static <T> void insertInOrder(Queue<T> q, T x,
        ...
        Queue<T> queueTempt = q.newInstance();

        while (q.length() != 0 && order.compare(x, q.front()) > 0) {
            queueTempt.enqueue(q.dequeue());
        }
        queueTempt.enqueue(x);
        queueTempt.append(q);
        q.transferFrom(queueTempt);
    ...
    public void sort(Comparator<T> order) {
        ...
        Queue<T> QueueTempt = this.newInstance();

        while (this.length() > 0) {
            T x = this.dequeue();
            insertInOrder(QueueTempt, x, order);
        }
        this.transferFrom(QueueTempt);
