...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (array.length > 1) {
            ...
            if (last >= rightIndex) {
                ...
                if (order.compare(root, smallerElement) > 0) {
            ...
            else if (last == leftIndex) {
                T leftElement = array[leftIndex];
                if (order.compare(root, leftElement) > 0) {
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        T root = array[top];
        int overallIndex = array.length - 1;
        ...
        if (overallIndex >= 1) {
            ...
            if (rightIndex <= overallIndex) {
                ...
                if (order.compare(root, rightElement) > 0) {
            ...
            else if (leftIndex == overallIndex) {
                T leftElement = array[leftIndex];
    ...
    private void createNewRep(Comparator<T> order) {
        //need to set this.entries equal to an empty queue.
