...
public final class BinarySearchTreeMethods {
    ...
    public static <T extends Comparable<T>> boolean isInTree(BinaryTree<T> t,
        ...
        if (height > 0) {
            ...
            if (root.compareTo(x) == 0) {
                ...
                contain = isInTree(lt, x);
            } else {
                contain = isInTree(rt, x);
            }
    ...
    public static <T> T removeSmallest(BinaryTree<T> t) {
        ...
        if (height == 1) {
            ...
            T smallestL = removeSmallest(left);
            T smallestR = removeSmallest(right);

            if (smallestL.compareTo(smallest) < 0) {
                smallest = smallestL;
            }
            if (smallestR.compareTo(smallest) < 0) {
                smallest = smallestR;
            }
