...
public final class Tokenizer {
    ...
    private static String nextWordOrSeparator(String text, int position) {
         ...
         * Returns the first "word" (maximal length string of characters not in
         * {@code SEPARATORS}) or "separator string" (maximal length string of
         * characters in {@code SEPARATORS}) in the given {@code text} starting
         ...
         * @param position
         *            the starting index
         * @return the first word or separator string found in {@code text}
        ...
        Set<String> separatorSet = new Set1L<String>();

        for (int i = 0; i < SEPARATORS.length(); i++) {
            String c = SEPARATORS.substring(i, i + 1);
            if (!separatorSet.contains(c)) {
                separatorSet.add(c);
            }
        }

        boolean isSeparatorFirst = separatorSet
                .contains(Character.toString(text.charAt(position)));

        int i = 0;
        boolean keepSeparator = separatorSet
                .contains(Character.toString(text.charAt(position + i)));
        while ((position + i) < text.length()
                && keepSeparator == isSeparatorFirst) {
            i++;
            if ((position + i) < text.length()) {
                keepSeparator = separatorSet.contains(
                        Character.toString(text.charAt(position + i)));
            }
        }
    ...
    public static Queue<String> tokens(SimpleReader in) {
        ...
        while (!in.atEOS()) {
            ...
            while (position < inputLine.length()) {
                String next = nextWordOrSeparator(inputLine, position);

                if (!separatorSet.contains(next)) {
                    tokens.enqueue(next);
                }
